...
struct ship {
	...
	void SortShip()
	{
		if (bigger(start.x, finish.x) || bigger(start.y, finish.y))
		{
			point a = start;
			start = finish;
			finish = a;
	...
	{
		bool res = false;
		if ((len > 0 && len < 5) && ((start.x == finish.x && abs(start.y - finish.y) == len - 1) || (start.y == finish.y && abs(start.x - finish.x) == len - 1)) && (start.x > 0) && (start.y > 0) && (start.x < 11) && (start.y < 11) && (finish.x > 0) && (finish.y > 0) && (finish.x < 11) && (finish.y < 11))
			res = true;
		return res;
	}
};
